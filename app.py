...
def get_value_by_path(data, path, allow_fallback=True):
    ...
    # Step 2: Fallbacks — only if not already tried
    if allow_fallback:
        fallback_paths = {
        ...
        if last_key in fallback_paths:
            return get_value_by_path(data, fallback_paths[last_key], allow_fallback=False)
